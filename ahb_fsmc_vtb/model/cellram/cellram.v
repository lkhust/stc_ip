...
`include "cellram_parameters.vh"
    ...
    reg  par_enabled;
    assign                        clk_in   = 1'b0;
    assign                        adv_n_in = 1'b0;
    assign                        cre_in   = 1'b0;
    assign                        zz_n_in  = zz_n;
    initial                       par_enabled <= 1'b0; // PAR enabled using ZZ# or by software access write
    ...
    always @(zz2pd) begin
        if (deep_power_down_n) begin // RCR[4] == 1
            // Once the software sequence loads the RCR, the ZZ# level no longer enables PAR operation.
            if (!software_access_rcr_write) begin
                if (zz2pd) begin
                    if (DEBUG[0])
                        $display ("%t  INFO: Async - Partial Array Refresh Enabled using ZZ#", $realtime);
                    par_enabled <= 1'b1;
                end else begin
                    if (DEBUG[0])
                        $display ("%t  INFO: Async - Partial Array Refresh Disabled using ZZ#", $realtime);
                    par_enabled <= 1'b0;
                end
            end
        end else begin
            // DPD is initiated by bringing ZZ# LOW for longer than 10us.
            if (zz2pd) begin
                if (DEBUG[0])
            ...
            case(select)
                ...
                RCR: begin
                    ...
                    if (software_access_unlock) begin
                        ...
                        par_enabled <= 1'b1; // PAR permanently enabled, independent of ZZ# level.
                        software_access_rcr_write = 1'b1;
                    end
